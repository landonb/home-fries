...
home_fries_fix_terminal_colors () {
  ...
  if [ -e /etc/os-release ]; then
    ...
    elif [ "$(cat /etc/os-release | grep -q "^ID=debian\$")" ]; then
      # OS: Debian
      # SAVVY/2025-01-14: ORIG: From `dircolors --sh`, unchanged:
    ...
    # # ISOFF/2025-01-14: It's been years since this settings has been
    # #  captured, much less verified. So disabling until revived.
