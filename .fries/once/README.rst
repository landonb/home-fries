...
There are some tasks that are not easily automated, such as
installing browser plugins and configuring MATE panels, and
some tasks that are deliberately not automated. Perform these
steps yourself, if you want.

Manual MATE Configuration
^^^^^^^^^^^^^^^^^^^^^^^^^
